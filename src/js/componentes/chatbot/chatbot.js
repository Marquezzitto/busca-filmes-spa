...
export function inicializarChatbot() {
  ...
  const enviarMensagem = async () => {
    ...
    // Envia o histórico completo para dar MEMÓRIA à IA
    const resposta = await perguntarOllama(historico);

    // Remove balão de carregando
    const loadingBubble = document.getElementById(loadingId);
    if (loadingBubble) loadingBubble.remove();

    // Adiciona a resposta da IA no histórico e salva
  ...
  // Renderiza no carregamento inicial
  renderizarMensagens();
}
